...
## Context
Verity Portal is an evolving ecosystem of compliance modules (Identity, Intake, Compliance, Analytics). Initially, we adopted a pure Hexagonal (Ports & Adapters) architecture to protect business logic. However, as the number of features increased, the top-level separation into `domain/` and `infrastructure/` directories led to "feature scattering," where adding a single capability required jumping between multiple distant directories.
...
## Decision
We decided to transition the backend architecture from a top-level Layered/Hexagonal split to a **Feature-Based Layout (Vertical Slicing)**. 
...
2. **Internal Feature Layout**: Each feature directory contains its own localized layers:
   - `router.py`: API endpoints (External Adapter).
   - `service.py`: Business logic and orchestration (Core).
   - `models.py`: Database entities (Infrastructure Adapter).
...
### Pure Hexagonal Architecture (Top-Level)
...
- Rejected: While the *principles* of Hexagonal (Ports/Adapters) are still applied *within* a feature, the top-level directory split was too cumbersome for our development velocity.
