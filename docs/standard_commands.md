Task,Frontend (Angular 21),Backend (Poetry / FastAPI)
Install Dependencies,npm install,poetry install
Run Dev Server,npm start,poetry run uvicorn app.main:app --reload
Run Tests,npm test,poetry run pytest
Linting,npm run lint,poetry run ruff check .
Formatting,npm run format,poetry run black .
Add Dependency,npm install [pkg],poetry add [pkg]
